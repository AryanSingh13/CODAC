...
def generate_small_and_large_jobs(num_stream_jobs):
    all_t, _ = generate_jobs(num_stream_jobs)
    probs = [np.random.randint(low = 1, high = 92) for _ in range(1000)]
    new_sizes = [] 
    for prob in probs: 
        size = 100 
        if prob == 91:
    ...
    timeline = [] 
    for t, size in zip(all_t, new_sizes):
        timeline.append((t, size))
    
    return timeline

def generate_jobs_v2(num_stream_jobs):
    all_t, all_sizes = generate_jobs(num_stream_jobs)
    timeline = [] 
    for t, size in zip(all_t, all_sizes):
        timeline.append((t, size))
