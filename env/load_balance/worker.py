...
class Worker(object):
    ...
    def get_wait_time(self, wall_time): 
        wait_time = 0 
        if self.curr_job != None and self.curr_job.finish_time > wall_time: 
            wait_time += self.curr_job.finish_time - wall_time
        
        for job in self.queue: 
            wait_time += int(job.size / self.service_rate)
