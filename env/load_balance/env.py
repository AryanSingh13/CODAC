...
class LoadBalanceEnvironment(object):
    def __init__(self):
        ...
        # finished jobs
        self.finished_jobs = []
        self.event_timeline_copy = []

    def generate_jobs(self):
        all_t, all_size = generate_jobs(self.num_stream_jobs)
        for t, size in zip(all_t, all_size):
            self.timeline.push(t, size)
            self.event_timeline_copy.append((t, size))


    def generate_jobs_external(self, timeline): 
        for t, size in timeline:
            self.timeline.push(t, (int)(size))
            self.event_timeline_copy.append((t, size))
    ...
    def reset(self, external_timeline = None):
        for worker in self.workers:
            worker.reset()
        self.wall_time.reset()
        self.timeline.reset()
        if external_timeline is None:
            self.generate_jobs()
        else:
            self.generate_jobs_external(external_timeline)
